...
def open_dest(dest: str) -> Tuple[str, Callable[[str, Union[bytes, str]], None], Callable[[], None]]:
    ...
    if dest_ext == 'zip':
        ...
        if os.path.isdir(dest) and len(os.listdir(dest)) != 0:
            ...
            print('--dest folder must be empty')
            shutil.rmtree(dest, ignore_errors=True)
        os.makedirs(dest, exist_ok=True)

        image_list_file = f"{dest}_image_list.txt"
        image_list_f = logging_utils_v2.get_logger(image_list_file, logger_names=(), stream=False, mode='w')
        ...
        def folder_write_bytes(fname: str, data: Union[bytes, str]):
            ...
            if not fname.endswith('.json'):
                image_list_f.info_msg(fname)

        close_func = lambda : print(f"Save image list file: {image_list_file}")

        return dest, folder_write_bytes, close_func
...
@click.option('--debug', help='', type=bool, default=False)

  ...
  with torch.no_grad():
    for idx_b in range((num_imgs + batch_size - 1) // batch_size):
      ...
      for idx_i, img in enumerate(generated_imgs):
        ...
        save_image(img, saved_path, normalize=True, value_range=(-1, 1), scale_each=True)
...
def main():
  ...
  cam_param = cam_params.CamParams.from_config(num_imgs=140000, H0=H, W0=W).to(device)

  model_dict = {
    'G_ema': G_ema,
    'cam_param': cam_param,
  }
  torch_utils.load_models(save_dir=global_cfg.pkl_dir, model_dict=model_dict, rank=rank)

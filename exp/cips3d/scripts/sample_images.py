...
def sample_images(rank,
  ...
  metadata['h_stddev'] = 0
  metadata['v_stddev'] = 0
  metadata['h_mean'] = np.pi * 0.5 + 0.15

  generator.eval()

  if rank == 0:
    pbar = tqdm.tqdm(desc=f"{fake_dir} at {img_size}x{img_size}", total=num_imgs)
  with torch.no_grad():
    for idx_b in range((num_imgs + batch_size - 1) // batch_size):
      ...
      generated_imgs = generated_imgs.squeeze()

      saved_path  = f"{fake_dir}/{seed:0>10}.jpg"
      save_image(generated_imgs, saved_path, normalize=True, value_range=(-1, 1))

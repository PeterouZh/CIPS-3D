...
def save_images_horizontal_flip(saved_imgdir,
    ...
    fixed_z = fixed_z[:20]
    with torch.no_grad():
        with torch.cuda.amp.autocast(global_cfg.use_amp):
            ...
            gen_imgs1 = G_ema(fixed_z.to(device),
                              return_aux_img=True,
            ...
            # copied_metadata['img_size'] = 128
            gen_imgs2 = G_ema(fixed_z.to(device),
                              return_aux_img=True,
                              forward_points=128 ** 2,
                              **copied_metadata)[0]
...
def save_images(generator_ddp,
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast(global_cfg.use_amp):
            ...
            copied_metadata['h_stddev'] = copied_metadata['v_stddev'] = 0
            # copied_metadata['img_size'] = max(128, copied_metadata['img_size'])
            gen_imgs = G_ema(fixed_z.to(device),
                             return_aux_img=True,
                             forward_points=128 ** 2,
                             **copied_metadata)[0]
    save_image(gen_imgs,
...
def train(rank,
    ...
    scaler_G = torch.cuda.amp.GradScaler(global_cfg.use_amp)
    scaler_D = torch.cuda.amp.GradScaler(global_cfg.use_amp)
    state_dict = {
        'best_fid': np.inf,
    }
    ...
    optimizer_D.param_groups[0]['weight_decay'] = global_cfg.D_wd
    ...
    for _ in range (discriminator.epoch, opt.n_epochs):
        ...
        for i, (imgs, _) in enumerate(dataloader):
            ...
            if metadata['r1_lambda'] > 0:
                # Gradient penalty
                grad_real = torch.autograd.grad(outputs=scaler_D.scale(r_preds.sum()),
                                                inputs=real_imgs,
                                                create_graph=True)
            ...
            D_total_norm = torch.nn.utils.clip_grad_norm_(discriminator_ddp.parameters(), metadata['grad_clip'],
            ...
            G_total_norm = torch.nn.utils.clip_grad_norm_(generator_ddp.parameters(), metadata.get('grad_clip', 0.3),
                                                          error_if_nonfinite=False)

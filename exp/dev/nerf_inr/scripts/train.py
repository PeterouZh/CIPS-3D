...
def save_images_horizontal_flip(metadata,
                                generator_ddp,
                                fixed_z,
                                device,
                                step):
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            ...
            gen_imgs1 = generator_ddp.module.staged_forward(fixed_z.to(device), **copied_metadata)[0]

            copied_metadata = copy.deepcopy(metadata)
            copied_metadata['h_stddev'] = copied_metadata['v_stddev'] = 0
            copied_metadata['h_mean'] = 1.70
            copied_metadata['img_size'] = 128
            gen_imgs2 = generator_ddp.module.staged_forward(fixed_z.to(device), **copied_metadata)[0]

    gen_imgs = torch.cat([gen_imgs1[:25], gen_imgs2[:25]])
    save_image(gen_imgs,
               os.path.join(global_cfg.tl_imgdir, f"{step:06d}_flip_ema.jpg"),
               nrow=5,
               normalize=True)
    pass


def train(rank, world_size, opt, cfg):
    # set_global_cfg(cfg)
        ...
        for i, (imgs, _) in enumerate(dataloader):
            ...
            if rank == 0:
                ...
                if global_cfg.tl_debug or \
                      (i % global_cfg.get('log_every', 100) == 0) and discriminator.step > 50:
                    summary_dict = {
                        'd_loss': d_loss.item(),
                        'g_loss': g_loss.item(),
                        'alpha': alpha,
                ...
                if discriminator.step % global_cfg.get('log_img_every', 500) == 0:
                    ...
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_fixed.jpg"),
                               nrow=5,
                               normalize=True)
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(fixed_z.to(device),  **copied_metadata)[0]
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_tilted.jpg"),
                               nrow=5,
                               normalize=True)
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(fixed_z.to(device),  **copied_metadata)[0]
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_fixed_ema.jpg"),
                               nrow=5,
                               normalize=True)
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(fixed_z.to(device),  **copied_metadata)[0]
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_tilted_ema.jpg"),
                               nrow=5,
                               normalize=True)

                    save_images_horizontal_flip(metadata=metadata,
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(
                                ...
                                **copied_metadata)[0]
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_random.jpg"),
                               nrow=5,
                               normalize=True)
                ...
                # for test_id in range(10):
                    ...
                    saved_ckpt_dir = ckpt_max2keep.step_and_ret_circle_dir(
                        global_cfg.tl_ckptdir, info_msg=discriminator.step)
                    saved_models(saved_dir=saved_ckpt_dir,
                                 ema=ema,
                                 ema2=ema2,
                                 generator_ddp=generator_ddp,
            ...
            if (discriminator.step) % global_cfg.get('eval_every', 5000) == 0:
                ...
                if rank == 0:
                    fid_evaluation.setup_evaluation(metadata['dataset'],
                                                    ...
                                                    **global_cfg.curriculum.get('new_attrs', {}))
                dist.barrier()
                ema.store(generator_ddp.parameters())
                ema.copy_to(generator_ddp.parameters())
                generator_ddp.eval()
                fid_evaluation.output_images(generator_ddp,
                                             metadata,
                                             rank,
                                             world_size,
                                             generated_dir,
                ...
                if rank == 0:
                    ...
                    with open(os.path.join(opt.output_dir, f'fid.txt'), 'a') as f:
                        f.write(f'{discriminator.step}:{fid}\n')
                    summary_dict = {
                        'fid': fid
                    }
                    ...
                    if state_dict['best_fid'] > fid:
                        ...
                        saved_ckpt_dir = f"{global_cfg.tl_ckptdir}/best"
                        saved_models(saved_dir=saved_ckpt_dir,
                                     ema=ema,
                                     ema2=ema2,
                                     generator_ddp=generator_ddp,

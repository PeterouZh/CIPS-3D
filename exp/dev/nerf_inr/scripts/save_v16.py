...
def train(rank,
    ...
    metadata['h_mean'] = math.pi * 0.5
    metadata['v_mean'] = math.pi * 0.5
    metadata['h_stddev'] = 0
    metadata['v_stddev'] = 0

    pbar = range(global_cfg.num_eval_images // (world_size * metadata.batch_size))
    if rank == 0:
      pbar = tqdm(pbar, desc=f"{fake_dir} [{world_size * metadata.batch_size * len(pbar)}]")
    for b in pbar:
        with torch.no_grad():
            seed = np.random.randint(100000000)
            psi = global_cfg.min_psi + (1 - global_cfg.min_psi) * np.random.rand()
            psi = round(psi, 2)
            ...
            g_imgs, g_pos = generator(zs=zs,
                                      ...
                                      # grad_points=128**2,
                                      **{**metadata,
                                         'forward_points': cfg.forward_points ** 2,
                                         'psi': psi})
            g_imgs = (g_imgs + 1.) * 0.5
            for idx, img in enumerate(g_imgs):
                img_pil = trans_f.to_pil_image(img)
                save_path = f"{fake_dir}/seed_{seed}_psi_{psi:.2f}_rank{rank}_b{b}_idx{idx}.{global_cfg.get('ext', 'png')}"

...
def train(rank,
    ...
    if 'D_cfg' in global_cfg:
        discriminator = build_model(cfg=global_cfg.D_cfg,
                                    kwargs_priority=True).to(device)
    else:
    ...
    if metadata.get('unique_lr', False):
        ...
        if hasattr(generator, 'get_optimizer'):
            optimizer_G = generator.get_optimizer(**global_cfg.G_cfg.optim)
        else:
            optimizer_G = torch.optim.Adam(
    ...
    if not hasattr(generator, 'get_optimizer'):
    ...
    for _ in range (discriminator.epoch, opt.n_epochs):
        ...
        for i, (imgs, _) in enumerate(dataloader):
            ...
            # if global_cfg.use_amp_G:
            #     if scaler_G.get_scale() < 1:
            #         scaler_G.update(1.)
            # if global_cfg.use_amp_D:

...
class ResidualCCBlockFirstDown(nn.Module):
  def __init__(self,
    ...
    p = kernel_size // 2
    self.network = nn.Sequential(
      CoordConv(inplanes, planes, kernel_size=kernel_size, padding=p, stride=stride),
      nn.LeakyReLU(0.2, inplace=True),
    ...
    self.proj = nn.Conv2d(inplanes, planes, 1, stride=stride)
    pass
...
class DiscriminatorMultiScale(nn.Module):
  def __init__(self,
    ...
    self.layers = nn.ModuleList(
      [
        # 1024
        ResidualCCBlockFirstDown(3, 32),
        # 512
        ResidualCCBlockFirstDown(32, 64),
        # 256
        ResidualCCBlockFirstDown(64, 128),
        # 128
        ResidualCCBlockFirstDown(128, 256),
        # 64
        ResidualCCBlockFirstDown(256, 512),
        # 32
        ResidualCCBlockFirstDown(512, 512),
        # 16
        ResidualCCBlockFirstDown(512, 512, stride=1),
        # 16
        ResidualCCBlockFirstDown(512, 512, stride=1),
        # 16
        # ResidualCCBlock(400, 400),

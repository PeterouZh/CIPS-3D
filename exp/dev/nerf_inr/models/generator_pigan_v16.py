...
class NeRFNetwork(nn.Module):
  ...
  def __init__(self,
    ...
    _in_dim= hidden_dim
    _out_dim = hidden_dim
    self.color_layer_sine = film_layer.FiLMLayer(
...
@MODEL_REGISTRY.register(name_prefix=__name__)
class GeneratorPiGAN(GeneratorNerfINR_base):
  def __init__(self,
               z_dim,
               nerf_cfg,
               mapping_nerf_cfg,
    ...
    # inr_net
    self.inr_net = nn.Identity()
    self.mapping_network_inr = nn.Identity()

    self.aux_to_rbg = nn.Identity()
  ...
  def points_forward(self,
    ...
    inr_img = self.inr_net(pixels_fea)
...
@MODEL_REGISTRY.register(name_prefix=__name__)
class GeneratorNerfINR_freeze_NeRF(GeneratorPiGAN):

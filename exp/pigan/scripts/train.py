...
def train(rank, world_size, opt, cfg):
    ...
    fixed_z = z_sampler((global_cfg.get('fixed_z_bs', 25), 256), device='cpu', dist=metadata['z_dist'])
            ...
            if rank == 0:
                ...
                if discriminator.step % global_cfg.get('log_img_every', 500) == 0:
                    ...
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_fixed.png"),
                               nrow=5,
                               normalize=True)
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(fixed_z.to(device),  **copied_metadata)[0]
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_tilted.png"),
                               nrow=5,
                               normalize=True)
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(fixed_z.to(device),  **copied_metadata)[0]
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_fixed_ema.png"),
                               nrow=5,
                               normalize=True)
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(fixed_z.to(device),  **copied_metadata)[0]
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_tilted_ema.png"),
                               nrow=5,
                               normalize=True)
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(
                                ...
                                **copied_metadata)[0]
                    save_image(gen_imgs[:25],
                               os.path.join(global_cfg.tl_imgdir, f"{discriminator.step:06d}_random.png"),
                               nrow=5,
                               normalize=True)

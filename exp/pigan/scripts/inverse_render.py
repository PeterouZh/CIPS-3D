...
def main():
    ...
    gt_image = gt_image.resize((opt.image_size, opt.image_size), Image.LANCZOS)
    ...
    writer = skvideo.io.FFmpegWriter(
        os.path.join(opt.output_dir, output_name), outputdict={'-pix_fmt': 'yuv420p', '-crf': '21'})
    ...
    with torch.no_grad():
        for pitch, yaw in tqdm.tqdm(trajectory):

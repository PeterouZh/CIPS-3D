...
from exp.pigan.models.math_utils_torch import *
...
def perturb_points(points,
    ...
    points = points + \
             offset * ray_directions.unsqueeze(2)
...
def sample_camera_positions(device,
    ...
    if mode == 'uniform':
        theta = (torch.rand((n, 1), device=device) - 0.5) \
                * 2 * horizontal_stddev \
                + horizontal_mean
        phi = (torch.rand((n, 1), device=device) - 0.5) \
              * 2 * vertical_stddev \
              + vertical_mean

    elif mode == 'normal' or mode == 'gaussian':
        theta = torch.randn((n, 1), device=device)\
                * horizontal_stddev \
                + horizontal_mean
        phi = torch.randn((n, 1), device=device) \
              * vertical_stddev \
              + vertical_mean
        ...
        theta = truncated_normal_(torch.zeros((n, 1), device=device)) \
                * horizontal_stddev \
                + horizontal_mean
        phi = truncated_normal_(torch.zeros((n, 1), device=device)) \
              * vertical_stddev \
              + vertical_mean

    elif mode == 'spherical_uniform':
        theta = (torch.rand((n, 1), device=device) - .5) \
                * 2 * horizontal_stddev \
                + horizontal_mean

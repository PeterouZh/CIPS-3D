...
class Testing_ffhq_exp(unittest.TestCase):
  ...
  def test__plot_fid_ablations(self):
    """
    export PYTHONPATH=.:./tl2_lib
    python -c "from exp.tests.test_cips3d import Testing_ffhq_exp;\
      Testing_ffhq_exp().test__plot_fid()"
    ...
    dd[f'{bucket_root}/results/CIPS-3D/ffhq_exp/train_ffhq-20220125_125446_251'] = \
      {'20220125_125446_251-ffhq_r64-gpu.8x4-finetune.F-aux.F': f"{log_file}", }
    ...
    dd[f'{bucket_root}/results/CIPS-3D/ffhq_exp/train_ffhq-20220102_120440_740'] = \
      {'20220102_120440_740-ffhq_r128-gpu.8x4-aux.T': f"{log_file}", }
    dd[f'{bucket_root}/results/CIPS-3D/ffhq_exp/train_ffhq-20220125_163018_465'] = \
      {'20220125_163018_465-ffhq_r64-gpu.8x4-finetune.T-aux.F': f"{log_file}", }
  ...
  def test_sample_images(self, debug=True):
    """
    Usage:
        ...
        export PYTHONPATH=.:./tl2_lib
        python -c "from exp.tests.test_cips3d import Testing_ffhq_exp;\
          Testing_ffhq_exp().test_sample_images(debug=False)"
    ...
    cmd_str = f"""
        python -m torch.distributed.launch --nproc_per_node={n_gpus} --master_port={PORT} 
        exp/cips3d/scripts/sample_images.py
        {get_append_cmd_str(args)}
        """
    if debug:
      cmd_str += f"""
  ...
  def test_render_video_web(self, debug=True):
    """
    ...
    Usage:
        ...
        export PYTHONPATH=.:./tl2_lib
        python -c "from exp.tests.test_cips3d import Testing_ffhq_exp;\
          Testing_ffhq_exp().test_render_video_web(debug=False)" \
          --tl_opts port 8501
  ...
  def test_model_interpolation_web(self, debug=True):
    """
    ...
    Usage:
        ...
        export PYTHONPATH=.:./tl2_lib
        python -c "from exp.tests.test_cips3d import Testing_ffhq_exp;\
          Testing_ffhq_exp().test_model_interpolation_web(debug=False)" \
          --tl_opts port 8501
    ...
    # script = "tl2_lib/tl2/proj/streamlit/scripts/run_web.py"
    script = "scripts/web_demo.py"
    if debug:
      cmd_str = f"""
          python 
            {script}
            --outdir {args.tl_outdir}/exp
  ...
  def test__extract_frames(self, debug=True):
    ...
    # video_file = Path("datasets/videos/seed_593318_psi_1_00_inr_nerf_hd.mp4")
    # video_file = Path("datasets/videos/seed_13722416_psi_1_00_inr_hd.mp4")
    video_file = Path("datasets/videos/seed_13722416_psi_1_00_nerf_hd.mp4")
